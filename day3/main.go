...
func calcPart1(input string) (int64, error) {
	schematic := [][]string{}
	...
	grid := grid.New[string](schematic)

	// parse number sequences and replace each digit with the full number
	buf := int64(0)
	bufCoords := [][2]int{}
	for y, row := range schematic {
		...
		for x, c := range row {
			if isNumber(c) {
				...
				buf += num
				bufCoords = append(bufCoords, [2]int{x, y})
			} else {
				for _, coord := range bufCoords {
					grid.Set(coord[0], coord[1], fmt.Sprint(buf))
				}
				buf = 0
				bufCoords = [][2]int{}
			}
	...
	for y, row := range *grid {
		fmt.Println(row)
		// track the last number processed to ensure we don't add it multiple times
		lastNumber := int64(-1)
		for x, c := range row {
			if !isNumber(c) {
				if c == "." {
			...
			num, _ := strconv.ParseInt(c, 10, 64)
			tiles := grid.Adjacent(x, y)
			for _, adjacent := range tiles {
				if adjacent == nil {
					continue
				}

				if isSymbol(*adjacent) && (lastNumber == -1 || lastNumber != num) {
					total += num
					lastNumber = num
					break
				} else {
...
func calcPart2(input string) (int64, error) {
	schematic := [][]string{}
	for _, rows := range strings.Split(input, "\n") {
		row := []string{}
		for _, col := range rows {
	...
	for y, row := range *grid {
		...
		for x, c := range row {
			...
			for _, adjacent := range tiles {
				...
				if isNumber(*adjacent) {
					num, _ := strconv.ParseInt(*adjacent, 10, 64)
					numbers[num] = struct{}{}
				}
			}
			if len(numbers) != 2 {
...
func hasSymbol(schematic [][]string, foundNum string, x, y int) bool {
	numLen := len(foundNum)
	// horizontally
	// left
	if withinBounds(schematic, x-numLen, y) {
		symb := schematic[y][x-numLen]
		if symb != "." {
			return true
		}
	}
	// right
	if withinBounds(schematic, x+1, y) {
		symb := schematic[y][x+1]
		if symb != "." {
			return true
		}
	}
	// up
	for i := 0; i < numLen; i++ {
		if withinBounds(schematic, x-i, y-1) {
			symb := schematic[y-1][x-i]
			if symb != "." {
				return true
			}
		}
	...
	// up-left
	if withinBounds(schematic, x-numLen, y-1) {
		symb := schematic[y-1][x-numLen]
		if symb != "." {
			return true
		}
	}
	// up-right
	if withinBounds(schematic, x+1, y-1) {
		symb := schematic[y-1][x+1]
		if symb != "." {
			return true
		}
	}
...
func withinBounds(schematic [][]string, x, y int) bool {
	if y < 0 {
		return false
	}
	if y >= len(schematic) {
...
var sub1 = `.........232.633.......................803..........................361................192............539.................973.221...340.....
...
var part1 = `.........232.633.......................803..........................361................192............539.................973.221...340.....
.............*..............#.....256.#.........329....................*313............*.......766.......*..........472..-...........+..249.
670-..@.......181......814..865.........968......@.......605....128.............%......798.638...+....776...........*......%...........*....
.......563........741....*..........&.....-...............*........*....815*....921...........*..............428.219..993.584...990.431.....
.........................466.971..815...&....634.197.%...887....114.........521.........&..796.......-.......+..........*......*............
