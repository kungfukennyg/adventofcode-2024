...
func partTwo(input string) (int64, error) {
	...
	for _, line := range strings.Split(input, "\n") {
		...
		gameID, err := strconv.ParseInt(strings.Split(split[0], " ")[1], 10, 64)
		if err != nil {
			return 0, errors.Join(err, errors.New("failed to parse game id"))
		}

		fmt.Printf("Game %d\n", gameID)
		turns := strings.Split(split[1], ";")
		var maxRed, maxGreen, maxBlue int64
		for _, turn := range turns {
			for _, cubes := range strings.Split(turn, ",") {
				fmt.Printf(cubes + ",")

				parts := strings.Split(strings.TrimSpace(cubes), " ")
				num, err := strconv.ParseInt(parts[0], 10, 64)
				if err != nil {
					return 0, errors.Join(err, errors.New("failed to parse number of dice"))
				}
...
func partOne(input string, maxRed, maxGreen, maxBlue int) (int64, error) {
...
var part1 = `Game 1: 1 red, 5 blue, 10 green; 5 green, 6 blue, 12 red; 4 red, 10 blue, 4 green
